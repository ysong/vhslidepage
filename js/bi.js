...
var _BiStat = {
	record: function(appId, appName, openid,site_id,lng,lat){ // netType网络类型,lng经度,lat纬度
		var netType = _BiStat.getNetType();
		var nowDomain = new String(document.domain);
		if(nowDomain == 'cgtest.lightapp.cn' || nowDomain == 'tt.lightapp.cn' || nowDomain == 't.lightapp.cn' || !nowDomain.match(/www.lightapp.cn/i) || !nowDomain.match(/www.liveapp.cn/i)
		||!nowDomain.match(/liveapp.cn/)||!nowDomain.match(/lightapp.cn/)){
		...
		"&uuid="+v+'&nettype='+netType+'&openid='+openid+"&site_id="+site_id+'&x='+lng+'&y='+lat;
		_BiStat.ajaxUrl(url);
	},
